...
Automatisation du remplissage du rapport **TPFREP** (Terminal Departure and Performance Report, format UN/EDIFACT SMDG 3.0) à partir des fichiers opérationnels réels du terminal **SOMAPORT** : rapports de shift et listes de mouvements conteneurs (IMPORT/EXPORT MASTERYD).
...
│   ├── merge.py               # Fusion dynamique shift + MASTERYD et résolution navire/escale/voyage
...
| **📥 Traitement** | Upload glisser-déposer des rapports de shift et fichiers MASTERYD, sélection interactive en cas d'escales multiples, exécution du pipeline en 6 étapes avec progression visuelle et téléchargement des sorties (Excel  ). |
...
1. Déposez dans `data/input/` vos rapports de shift et fichiers MASTERYD (Import/Export).
...
| **Fichier MASTERYD** | Présence des colonnes `"Nø CONTENEUR"` et `"EXPLOITANT EN COURS"`. |
...
- **Contrôle de cohérence** : Vérification automatique entre le total des mouvements déclarés dans les shifts et le nombre d'enregistrements conteneurs dans les fichiers MASTERYD.
